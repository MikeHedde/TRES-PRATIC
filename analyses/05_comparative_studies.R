...
# 2. Agrégation
#############################################################
heatmap_data <- CS_db %>% #Tableau avec une colonne pour le nombre d'occurrences de chaque couple intervention/population
  #distinct(Comparative_study_code, Intervention_R2, Population_homogenized) %>% #ça ne change rien je pense car le comparative study code est déjà différent pour chaque ligne donc le distinct n'enlèvera pas de doublons
  count(Intervention_R2, Population_homogenized, name = "n") %>%
  na.omit() #on arrive bien à sum(heatmap_data$n) = 1386 : cohérent avec la bdd

intervention_counts <- CS_db %>% #>Tableau avec le nombre d'occurrences de chaque intervention
  #distinct(Study_ID, Intervention_R2) %>%
  count(Intervention_R2, name = "n") %>%
  na.omit()

pop_counts <- CS_db %>% #Tableau avec le nombre d'occurrences de chaque population
